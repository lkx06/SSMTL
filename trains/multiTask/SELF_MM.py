...
class SELF_MM():
    def __init__(self, args):
        assert args.train_mode == 'regression' or 'classification'
        ...
        while True: 
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    image_mask = batch_data['image_mask'].to(self.args.device)
                    audio = batch_data['audio'].to(self.args.device)
                    text = batch_data['text'].to(self.args.device)
                    text2=batch_data['text2'].to(self.args.device)
                    indexes = batch_data['index'].view(-1)
                    ...
                    # forward
                    #print(text2.shape)
                    outputs = model(text, (audio, audio_lengths), (vision,text2,image_mask))
                    # store results
