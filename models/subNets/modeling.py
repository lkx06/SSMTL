...
class BertForSequenceClassification(PreTrainedBertModel):
    ...
    def forward(self, text):
        input_ids, attention_mask, token_type_ids = text[:,0,:].long(), text[:,1,:].float(), text[:,2,:].long()
...
class VisBert(PreTrainedBertModel):
   
    def __init__(self, config,in_size, hidden_size, out_size, num_layers=1, dropout=0.2, bidirectional=False):
        super(VisBert, self).__init__(config)
        self.bert = BertModel(config)
    ...
    def forward(self, img_feat,text2,image_mask):
        input_ids, attention_mask, token_type_ids = text2[:,0,:].long(), text2[:,1,:].float(), text2[:,2,:].long()
        batch_size, seq = input_ids.size()
        _, roi_num, feat_dim = img_feat.size()
        sequence_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        ...
        _, final_states = self.rnn(visual_output)
        h = self.dropout(final_states[0].squeeze())
        
        text_img_output = torch.cat((h, pooled_output), dim=1)
        y_1 = self.linear_1(text_img_output)
        
        #pooled_output = self.dropout(final_text_output)

        return y_1,pooled_output
